...
void menu() 
{
    ...
    while (fp.read((char * ) & pr, sizeof(product))) 
    {
        cout << pr.retpno() << "\t\t" << pr.retname() << "\t\t" << pr.retprice()<< "\t\t\t"<<pr.retqty()<<"\t\t\t"<<pr.retcp()<< "\t\t\t"<<pr.retdis() <<"\t\t" << endl;
    }
    fp.close();
}
...
void place_order() 
{
    ...
    //clrscr();
    cout << "\n\n******************************** INVOICE ************************\n";
    cout << "\nPr No.\tPr Name\tQuantity \tPrice \tAmount \t Discount%\tAmount after discount\ n ";
    for (int x = 0; x <= c; x++) 
    {
        fp.open("Shop.dat", ios:: in );
        ...
        while (!fp.eof()) 
        {
            if (pr.retpno() == order_arr[x]) 
            {
                ...
                cout << "\n" << order_arr[x] << "\t" << pr.retname() <<
                    "\t" << quan[x] << "\t\t" << pr.retprice() << "\t" << amt << "\t\t" <<pr.retdis()<<"\t"<< damt;
                total += damt;
            }
            fp.read((char * ) & pr, sizeof(product));
        }
